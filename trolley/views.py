...
def adjust_trolley(request, item_id):
    ...
    if quantity > 0:
        if quantity < 100:
            trolley[item_id] = quantity
            messages.success(request, f'Quantity and Subtotal updated for {product.name}')
        else:
            trolley[item_id] =  99
            messages.warning(request, 'Quantity cannot be higher than 99 items')
    else:
        trolley.pop(item_id)
        messages.error(request, 'Item removed as the quantity was lower than 1 item')
